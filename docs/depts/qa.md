...
|:-----|:-----|:---------------|:--------------------------|
| **Functional** | End-to-end tab shell + Discover + Profile on a native build. | [Maestro](https://maestro.mobile.dev/) — [`testing/maestro/smoke-test.yaml`](../../testing/maestro/smoke-test.yaml) (includes [`auth-flow.yaml`](../../testing/maestro/auth-flow.yaml)); run **`npm run test:smoke-maestro`** with **`MAESTRO_TEST_USER_*`** set. | Flow completes without timeout; **`discover-smoke-poster`** appears; **`Your Cinematic Profile`** visible after **Profile**. Device **`applicationId`** matches **`smoke-test.yaml`** (**`com.reeldive.app`**). |
